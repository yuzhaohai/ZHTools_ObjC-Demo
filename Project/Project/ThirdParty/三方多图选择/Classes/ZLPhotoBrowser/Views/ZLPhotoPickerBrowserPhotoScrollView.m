...
- (void)setPhoto:(ZLPhotoPickerBrowserPhoto *)photo{
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            // 网络URL
//            [_photoImageView sd_setImageWithPreviousCachedImageWithURL:photo.photoURL placeholderImage:thumbImage options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
//            }];
            ...
            [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:thumbImage options:SDWebImageRetryFailed context:@{@"SDWebImageContextOption":SDWebImageContextImageCache} progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                [self setProgress:(double)receivedSize / expectedSize];
