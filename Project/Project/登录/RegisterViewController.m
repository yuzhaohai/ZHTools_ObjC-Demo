...
- (void)viewDidLoad {
    ...
    NSArray *array = @[@"请输入手机号", @"请输入验证码", @"请输入邀请码", @"请输入密码", @"确认密码"];
    NSArray *imageArray = @[@"phone", @"yzm", @"yzm", @"password", @"password"];
...
- (void)tappedCodeButton:(UIButton *)button {
    ...
    [Util POST:@"/api/Login/register_yzm" showHUD:YES showResultAlert:YES parameters:@{@"phone": textField.text} result:^(id responseObject) {
        if (responseObject) {
...
- (void)tappedButton:(UIButton *)button {
    ...
    parameters[@"check_code"] = textField.text;
    
    textField = self.viewArray[3];
    ...
    if (![textField.text isEqualToString:((UITextField *)self.viewArray[4]).text]) {
    ...
    [Util POST:@"/api/Login/register" showHUD:YES showResultAlert:YES parameters:parameters result:^(id responseObject) {
        if (responseObject) {
            [self goBack];
        }
    }];
